...
# Scriptin adı: ping_checker.sh
# Çalıştırmak için: chmod +x ping_checker.sh && ./ping_checker.sh

CONFIG_FILE="/etc/ping_checker.conf"
SERVICE_FILE="/etc/systemd/system/ping_checker.service"
SCRIPT_FILE="/usr/local/bin/ping_checker_script.sh"
...
install_script() {
    clear
    echo -e "${GREEN}Ping Checker Kurulum${NC}"
...
[Unit]
Description=Ping Checker Service
...
EOL
    ...
    systemctl enable ping_checker.service
    systemctl start ping_checker.service
...
uninstall_script() {
    clear
    echo -e "${GREEN}Ping Checker Kaldırma${NC}"
    
    # Servisi durdur ve devre dışı bırak
    systemctl stop ping_checker.service 2>/dev/null
    systemctl disable ping_checker.service 2>/dev/null
...
while true; do
    clear
    echo -e "${GREEN}Ping Checker Script${NC}"
    echo "1. Kurulum"
