...
BASE_DIR="/opt/ssh_tunnel2"
LOGFILE="$BASE_DIR/ssh_tunnel2.log"
RUNNER="$BASE_DIR/tunnel_runner2.sh"
SERVICE_FILE="/etc/systemd/system/ssh_tunnel2.service"

function start_tunnel() {
    read -p "VPS IP adresini girin: " VPS_IP
    read -p "Bağlantı portunu girin (örnek: 8080): " PORT
...
[Unit]
Description=SSH Reverse Tunnel 2
...
EOF
    ...
    sudo systemctl enable ssh_tunnel2.service
    sudo systemctl start ssh_tunnel2.service
...
function stop_tunnel() {
    ...
    sudo systemctl stop ssh_tunnel2.service
    sudo systemctl disable ssh_tunnel2.service
